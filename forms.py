...
class loginForm(FlaskForm):
    ...
    password = PasswordField(
        'Password', [validators.DataRequired(message="El pasword es requerido")])
...
class RegistroForm(FlaskForm):
    ...
    confirmar_contrasenia = PasswordField('Confirmar Contraseña', [validators.DataRequired(message="El campo es requerido"),
                                                                   validators.EqualTo(
                                                                       'contrasenia', message='Las contraseñas deben coincidir.')
                                                                   ])
...
class EditarPerfilForm(FlaskForm):
    nombre = StringField(
        'Nombre', [validators.DataRequired(), validators.Length(min=2, max=100)])
    nombre_usuario = StringField('Nombre de Usuario', [
                                 validators.DataRequired(), validators.Length(min=4, max=50)])
    telefono = StringField(
        'Teléfono', [validators.DataRequired(), validators.Length(min=8, max=15)])
    email = StringField('Correo Electrónico', [
                        validators.DataRequired(), validators.Email()])
    contrasenia = PasswordField('Nueva Contraseña (opcional)', [
