...
class RecetaForm(FlaskForm):
    ...
    cantidad_produccion = IntegerField('Cantidad Producida', [  
        ...
        validators.NumberRange(min=0, max=200, message='La cantidad debe ser mayor a 0')
    ])
    ...
    descripcion = StringField('Descripción de la galleta', [
        validators.DataRequired(message='Los detalles son requeridos'),
        validators.Length(min=1, max=250, message='La descripción debe tener entre 1 y 250 caracteres') 
    ])

    id_galleta = HiddenField()
    id_receta = SelectField('Seleccionar Galleta', coerce=int, choices=[], validators=[DataRequired()])
    ...
    imagen_galleta = FileField('Imagen', validators=[FileAllowed(['jpg', 'jpeg', 'png'], 'Solo imágenes')])

    def cargar_opciones(self):
        from models import Galleta
        self.id_receta.choices = [(g.id_galleta, g.nombre) for g in Galleta.query.all()]
