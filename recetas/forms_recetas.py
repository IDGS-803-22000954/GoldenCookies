...
class RecetaForm(FlaskForm):
    ...
    id_galleta = SelectField('Seleccionar Galleta', coerce=int, choices=[], validators=[DataRequired()])

    def cargar_opciones(self):
        with current_app.app_context():  # Asegurar que se tiene un contexto activo
            self.id_galleta.choices = [(g.id_galleta, g.nombre) for g in db.session.query(Galleta).all()]
