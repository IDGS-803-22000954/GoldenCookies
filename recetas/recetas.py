        ...
        if insumo_id.startswith('insumo_'):
            ...
            try:
                cantidad_insumo = float(
                    request.form[f'cantidad_{id_insumo}'].replace(',', '.'))
            except ValueError:
                cantidad_insumo = 0.0
...
def modificar_receta(id_receta):
    ...
    for insumo_id in request.form:
        if insumo_id.startswith('insumo_'):
            id_insumo = int(insumo_id.split('_')[1])
            cantidad_insumo = float(request.form.get(
                f'cantidad_{id_insumo}', '0').replace(',', '.'))
            nuevo_insumo = RecetaInsumo(
                id_receta=id_receta, id_insumo=id_insumo, cantidad_insumo=cantidad_insumo)
